    ...
    <Section id="features">
      <div className="container relative z-2">
        ...
        <div className="flex flex-wrap gap-10 mb-10 items-center justify-center">
          {benefits.map((item) => (
            <div key={item.id} className="block relative p-0.5 bg-no-repeat bg-[length:100%_100%] w-[90%] md:max-w-[45%] lg:max-w-[30%]" style={{ backgroundImage: `url(${item.backgroundUrl})` }}>
              ...
              <div className="absolute inset-0.5 bg-n-8" style={{ clipPath: "url(#benefits)" }}>
                <div className="absolute inset-0 opacity-0 transition-opacity hover:opacity-10">
                  {item.imageUrl && (
                    <img src={item.imageUrl} alt={item.title} width={380} height={362} className="size-full object-cover" />
                  )}
                </div>
              </div>
              <ClipPath />
